...
/*   Updated: 2023/12/24 11:50:03 by rvarela          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*line;
	static char	*buffer_tmp[FOPEN_MAX];

	if (fd < 0 || BUFFER_SIZE <= 0 || fd > FOPEN_MAX)
		return (NULL);
	buffer_tmp[fd] = ft_get_line(fd, buffer_tmp[fd]);
	if (!buffer_tmp[fd])
		return (NULL);
	line = get_till_n(buffer_tmp[fd]);
	buffer_tmp[fd] = ft_get_last(buffer_tmp[fd]);
	return (line);
}
