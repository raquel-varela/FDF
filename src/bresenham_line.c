...
/*   Updated: 2024/04/26 15:48:33 by rvarela-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	bresenham_param(t_bresenham *param, t_point *a, t_point *b)
{
	...
	if (b->y < a->y)
		param->step_y *= -1;
	param->p = param->dx + param->dy;
	param->x0 = a->x;
	param->y0 = a->y;
}

/*static void	slope_incr_x(t_map *data, t_point *a, t_point *b, t_bresenham *param)
{
...
static void	slope_incr_y(t_map *data, t_point *a, t_point *b, t_bresenham *param)
{
...
void	draw_line(t_map *data, t_point *a, t_point *b)
{
	...
	while (1)
	{
		...
		if (param.x0 == b->x && param.y0 == b->y)
			break ;
		if (2 * param.p >= param.dy && param.x0 != b->x)
		{
			param.p += param.dy;
			param.x0 += param.step_x;
		}
		if (2 * param.p <= param.dx && param.y0 != b->y)
		{
			param.p += param.dx;
			param.y0 += param.step_y;
		}
